...
//	int* ptr1 = (int*)(&aa + 1);//&aa取出二维数组的地址+1 -- 跳过了一整个二维数组
//	int* ptr2 = (int*)(*(aa + 1));//aa -- 第一行的地址 + 1 -- 跳过一行.*(aa + 1) == aa[1] -- 第二行数组名，第二行首元素的地址 -- 6的地址
...
//int main()
//{
//	char* a[] = { "work","at","alibaba" };//a[]字符指针数组w a a
//	char** pa = a;//数组名表示首元素地址
//	pa++;
//	printf("%s\n", *pa);//%s,以a的地址开头打印一个字符串 -- at
//	return 0;
//
//}
//int main()
//{
//	char* p = "abcdef";//把常量字符串abcdef的首字符的地址a放到p里
//	return 0;
//}
/*
	||=======================================================||
	||                                                       ||
	||       a -- char**首元素 --> char* --> w ork                ||
	||   a                pa++ --> char* --> a t                  ||
	||                             char* --> a libaba             ||
	||                                                       ||
	||   int*p p+1 --> int --> 4                             ||
	||   char*p p+1 --> char --> 1                           ||
	||   double*p p+1 --> double --> 8                       ||
	||   char** pa pa+1 --> char* --> 跳过一个char*          ||
	||                                                       ||
	||=======================================================||
*/
...
/*
									  ...
									  char***         char**        char*
													...
													|   c   | --> | char* | --> E NTER
									|---------|		|-------|     |-------|
						第二问++	|  ++cpp  |	-->	| c + 1 | --> | char* | --> N EW
									|---------|		|-------|     |-------|
		第四问cpp - 1	第一问++	|  ++cpp  |	-->	| c + 2 | --> | char* | --> P OINT
									|---------|     |-------|     |-------|
       	第三问cpp - 2				|   cpp   | --> | c + 3 | --> | char* | --> F IRST
									|---------|     |-------|	  |-------|


		第一问：**++cpp --> *(c + 2) --> POINT
		第二问：*--*++cpp + 3 --> *--(c + 1) + 3 --> *c + 3 --> ENT ER --> ER
		第三问：*cpp[-2] + 3 --> **(cpp - 2) + 3 --> *(c + 3) + 3 --> FIR ST --> ST
		第四问：cpp[-1][-1] + 1 --> *(*(cpp - 1)-1) + 1 --> *((c + 2)-1) + 1 --> *(c + 1) + 1 -->N EW --> EW
*/